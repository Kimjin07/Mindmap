...
/* 纯装饰:确定性星野(黄金角均匀散布,避免随机导致的水合抖动)。 */
const DECO_STARS = Array.from({ length: 150 }, (_, i) => {
  const a = i * 2.39996; // 黄金角
  const r = 1600 + ((i * 937) % 5200);
  return {
    x: Math.round(Math.cos(a) * r * 1.25),
    y: Math.round(Math.sin(a) * r * 0.85) - 200,
    s: 2 + ((i * 7) % 8),
    o: 0.2 + ((i * 13) % 45) / 100,
    gold: i % 7 === 0,
  };
    ...
    >
      ...
      >
        <svg
          ...
          aria-hidden="true"
        >
          {/* 装饰层:星野 + 银河带 + 环绕核心大陆的「护城河」(纯装饰,不拦截交互) */}
          {!tour && (
            <g className="atlas-deco" pointerEvents="none">
              <defs>
                <linearGradient id="galaxyBand" x1="0" y1="0" x2="1" y2="1">
                  <stop offset="0%" stopColor="#7fa8ff" stopOpacity="0.03" />
                  <stop offset="45%" stopColor="#e8c96a" stopOpacity="0.07" />
                  <stop offset="100%" stopColor="#7fa8ff" stopOpacity="0.03" />
                </linearGradient>
              </defs>
              {/* 银河带:斜跨整张地图,内窄外宽两层 */}
              <ellipse cx={300} cy={-200} rx={7200} ry={2100} transform="rotate(-24 300 -200)" fill="url(#galaxyBand)" />
              <ellipse cx={300} cy={-200} rx={7200} ry={950} transform="rotate(-24 300 -200)" fill="url(#galaxyBand)" />
              {/* 星野 */}
              {DECO_STARS.map((s, i) => (
                <circle key={i} cx={s.x} cy={s.y} r={s.s} className={s.gold ? "deco-star gold" : "deco-star"} style={{ opacity: s.o }} />
              ))}
              {/* 护城河:环绕五大核心层,扩展板块隔河相望 */}
              <circle cx={0} cy={-150} r={4120} className="moat-water" />
              <circle cx={0} cy={-150} r={4120} className="moat-flow" />
              <circle cx={0} cy={-150} r={3985} className="moat-ripple" />
              <circle cx={0} cy={-150} r={4255} className="moat-ripple" />
            </g>
          )}
